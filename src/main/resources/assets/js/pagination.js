(function($) {
	...
	Pagination.prototype = {
	    ...
	    bandEvent:function(){
	      ...
	      self.EL.delegate(".sortBy","click",function(e){
	    	  var target = $(e.currentTarget);
	    	  var pageNum =  self.EL.find(".paginator").attr("p_currentpagenum");
	    	  var sortBy = target.attr("sortBy");
	    	  var sortType = null;
	    	  self.options.paramJson.pageNum=pageNum;
		      if(sortBy){
		    	  if(target.hasClass("fa-sort")||target.hasClass("fa-caret-up")){
		    		  target.removeClass("fa-sort").removeClass("fa-caret-up").addClass("fa-caret-down");
		    		  sortType="desc";
		    		  self.options.paramJson.sortBy = sortBy+" desc";
		    	  }else if(target.hasClass("fa-caret-down")){
		    		  target.removeClass("fa-caret-down").addClass("fa-caret-up");
		    		  sortType="asc";
		    		  self.options.paramJson.sortBy = sortBy+" asc";
		    	  }
		      }
		    	...
		    	var url = self.options.url;
			     $.ajax({
			 		type : 'POST',
			 		url : url,
			 		data : self.options.paramJson,
			 		success : function(result) {
			 			self.EL.html(result);
			 			if(sortBy){
			 				var length = self.EL.find(".sortBy[sortby="+sortBy+"]").length;
			 				if(length>0){
			 					if("asc"==sortType){
			 						self.EL.find(".sortBy[sortby="+sortBy+"]").removeClass("fa-sort").removeClass("fa-caret-down").addClass("fa-caret-up");
			 					}else if("desc"==sortType){
			 						self.EL.find(".sortBy[sortby="+sortBy+"]").removeClass("fa-sort").removeClass("fa-caret-up").addClass("fa-caret-down");
			 					}
			 				}
			 			}
			 		},
