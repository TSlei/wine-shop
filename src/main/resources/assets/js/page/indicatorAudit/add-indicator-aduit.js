...
var setting = {
	    ...
	    async: {  
	        ...
	        otherParam:{"applyType" : "1"},//提交的其他参数,json的形式
...
function getAllParam(){
	...
	var sourceName = $("#sourceName").val()==""?null:$("#sourceName").val();
	var sourceUrl = $("#sourceUrl").val()==""?null:$("#sourceUrl").val();
	var jsonData = {
			indexCode:indexCode,
...
function loadTableData(catoId){
	...
	jsonData["applyType"]=1;
  	$("#addAuditTable").pagination({
			url : "/gldata/indicatorAudit/poi/queryAddAuditTable",
			paramJson : jsonData,
			callback:function(){
...
function checkedInput(type){
	...
	var checkboxs = $("#addAuditTable").find("[type='checkbox']:checked");
