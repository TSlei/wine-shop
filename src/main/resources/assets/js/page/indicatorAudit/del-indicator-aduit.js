...
var setting = {
	    ...
	    async: {  
	        ...
	        otherParam:{"applyType" : "3"},//提交的其他参数,json的形式
...
function loadTableData(catoId){
	...
	if(catoId!=null&&catoId!=undefined){
  	...
  	$("#delAuditTable").pagination({
			url : "/gldata/indicatorAudit/poi/queryDelAuditTable",
			paramJson : jsonData,
			callback:function(){
...
function checkedInput(type){
	...
	var checkboxs = $("#delAuditTable").find("[type='checkbox']:checked");
	for (var i = 0; i < checkboxs.length; i++) {
		...
		var dateList = $(checkboxs[i]).attr("dateList");
