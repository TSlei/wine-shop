...
function loadTableData(){
	...
	jsonData['pageSize']=15;
  	$("#addApplyTable").pagination({
			url : "/gldata/mycategory/queryAddApplyTable",
			paramJson : jsonData,
			callback:function(){
...
function uploadImportFile(){
    ...
    ({
			type:"POST",
            url: '/gldata/mycategory/addApplyImport', //用于文件上传的服务器端请求地址
            			      ...
            			      down: {
            			        ...
            			        callback: function () {
            			        	if(fileId){
            			        		...
            			        		window.location.href = "/gldata/io/download/log?fileId="+fileId+"&fileName=新增框架申请失败日志.xls";
            			        		setTimeout(function(){deleteImportLog(fileId);},1000);
            			        	}
            			        	bootbox.hideAll();
            			        }
...
$(document).ready(function () {
	...
	$(".downloadTemplate").on("click",function(){
	    window.location.href="/gldata/io/download/template?code=10001";
	});
...
$("#add-reset").click(function(){
	window.location.href="/gldata/mycategory/addApply";
});
