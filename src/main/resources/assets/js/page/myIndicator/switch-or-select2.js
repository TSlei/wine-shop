...
function selectModifyStatus(_this, value){
	switch (value) {
	case "1":
		$('.fn-radio2-hock-1').addClass('hidden').find("input").attr("disabled", "disabled"); //防止名称重复 form表单提交
		$('.fn-radio2-hock-0').removeClass('hidden').find("input").removeAttr("disabled");
		break;
	case "0":
		$('.fn-radio2-hock-0').addClass('hidden').find("input").attr("disabled", "disabled");
		$('.fn-radio2-hock-1').removeClass('hidden').find("input").removeAttr("disabled");
		break;
	default:
		break;
	}
